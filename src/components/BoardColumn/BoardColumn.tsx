...
const BoardColumn: FC<BoardColumnProps> = props => {
        ...
        if (currentBoard.columns) {
          ...
          if (draggedColumn && hoveredColumn) {
            const handleUpdateColumn = async (
              ...
              await updateColumn(currentBoard._id, draggedColumnId, draggedColumn.title, hoveredColumn.order);
              await updateColumn(currentBoard._id, hoveredColumnId, hoveredColumn.title, draggedColumn.order);
              const updatedColumns = ((await getAllColumns(currentBoard._id)) as ColumnData[]).sort(
                (a, b) => a.order - b.order,
              );
  ...
  const handleUpdateTask = async (
    ...
    const targetColumnData = (await getColumn(currentBoard._id, columnToDropId)) as ColumnData;
    ...
    const updatedTasks = ((await getAllTasks(currentBoard._id, dragColumnIndex)) as TaskData[]).sort(
      (a, b) => a.order - b.order,
    );
    dispatch(
      setColumnTaskData({
  ...
  const swapTasks = async (
    ...
    const updatedTasks = ((await getAllTasks(currentBoard._id, draggedTaskColumnId)) as ColumnData[]).sort(
      (a, b) => a.order - b.order,
    );
    dispatch(
      setColumnTaskData({
  ...
  const handleDeleteColumn = async (resp: boolean): Promise<void> => {
    if (resp) {
      await deleteColumn(currentBoard._id, props.columnId);
      dispatch(
        setDeleteColumn({
          columnId: props.columnId,
  ...
  const updateColumnTitle = async () => {
    if (newColumnTitle && newColumnTitle !== props.columnTitle) {
      await updateColumn(currentBoard._id, props.columnId, newColumnTitle, props.columnOrder);
      dispatch(
        setColumn({
          columnId: props.columnId,
  ...
  useEffect(() => {
    const getTasks = async (): Promise<void> => {
      if (currentBoard._id && props.columnId) {
        const data = ((await getAllTasks(currentBoard._id, props.columnId)) as TaskData[]).sort(
          (a, b) => a.order - b.order,
        );
        dispatch(
          setColumnTaskData({
    ...
    getTasks();
  }, [currentBoard._id, dispatch, props.columnId]);
